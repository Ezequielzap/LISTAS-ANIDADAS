#
              #0              #1             #2             #3
estanteria = [
                [["to12",65],["to16",86],["to20",65],["to25",45]],
                [["to30",68],["to35",73],["to40",85],["to45",89]], 
                [["ta4",58],["ta5",48],["ta6",64],["ta7",96]], 
                [["ta8",36],["ta10",72],["ta12",78],["ta14",71]]]




seguir = "s"
while seguir == "s":
    print("menu de opciones: \n1- Reponer mercadería (productos existentes) \n2- Vender mercadería (producto existente, solo si alcanza el stock) \n3- Listar inventario \n4- Salir")
    opcion = int(input("seleccione opcion: "))

    match opcion:
        case 1:
            for i in range(len(estanteria)):
                for j in range(len(estanteria[i])):
                    print("fila: ", i,"    columna: ", j,"    producto: ", estanteria[i][j])
            columna = int(input("en que columna esta el producto: "))
            fila = int(input("en que fila esta el prodructo: "))

            nueva_cantidad = int(input("cuanta mercaderia quiere reponer: "))
            estanteria[fila][columna][1] += nueva_cantidad
            print(estanteria[fila])
        case 2:
            for i in range(len(estanteria)):
                for j in range(len(estanteria[i])):
                    print("fila: ", i,"    columna: ", j,"    producto: ", estanteria[i][j])
            columna = int(input("en que columna esta el producto que quiere comprar: "))
            fila = int(input("en que fila se encuentra el producto que quiere comprar: "))
            cantidad_de_compra = int(input("cuanta cantidad quiere comprar ?: "))

            if cantidad_de_compra > estanteria[fila][columna][1]:
                print("no tenemos suficiente stock")
            else:
                estanteria[fila][columna][1] -= cantidad_de_compra
            print(estanteria[fila])
        case 3:
            for i in range(len(estanteria)):
                for j in range(len(estanteria[i])):
                    print("fila: ", i,"    columna: ", j,"    producto: ", estanteria[i][j])
        case 4:
            seguir = "n"
